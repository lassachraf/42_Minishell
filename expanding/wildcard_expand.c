...
/*   Updated: 2024/06/22 21:49:40 by alassiqu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void process_directory(char *dir_name, char **result) {
    ...
    if (!dir)
        return;
    dir_header = malloc(len + 3);
    ft_strcpy(dir_header, dir_name);
    dir_header[len] = ':';
    dir_header[len + 1] = '\n';
    dir_header[len + 2] = '\0';
    append_to_result(result, dir_header);
    free(dir_header);

    entry = readdir(dir);
    while (entry) {
        if (entry->d_name[0] != '.') {
            // Add two spaces before the entry name for indentation
            char *indented_entry = malloc(ft_strlen(entry->d_name) + 3);
            sprintf(indented_entry, "  %s", entry->d_name);
            append_to_result(result, indented_entry);
            free(indented_entry);
        }
        entry = readdir(dir);
    }
